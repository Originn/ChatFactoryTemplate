...
export default async function handler(
  ...
  try {
    ...
    const minScoreSourcesThreshold =
      process.env.MINSCORESOURCESTHRESHOLD !== undefined
        ? parseFloat(process.env.MINSCORESOURCESTHRESHOLD)
        : 0.86;

    const pdfResults = await filteredSimilaritySearch(
      vectorStore,
      (Documents[0] as any).responseText,
      'pdf',
      2,
